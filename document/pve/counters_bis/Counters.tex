...
\usepackage[orientation=portrait,size=custom,width=16,height=20,scale=0.5,debug]{beamerposter} 
...
\draw [line width=6.pt,color=#1] (0.,0.)-- (#2/180,0.);
...
\newcommand*{\dps}[1]{
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]\clip(-0.1,-0.1) rectangle (1.8,0.1);
\draw [line width=6.pt,color=black] (0.,0.)-- (#1/12.,0.);
\draw[color=white] (0.3,0.) node {\scriptsize{$#1$}};
\end{tikzpicture}
}
\newcommand*{\survival}[1]{
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]\clip(-0.1,-0.1) rectangle (1.8,0.1);
\draw [line width=6.pt,color=black] (0.,0.)-- (#1/25.,0.);
\draw[color=white] (0.3,0.) node {\scriptsize{$#1$}};
\end{tikzpicture}
}
\newcommand*{\tdo}[1]{
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]\clip(-0.1,-0.1) rectangle (1.8,0.1);
\draw [line width=6.pt,color=black] (0.,0.)-- (#1/390.,0.);
\draw[color=white] (0.3,0.) node {\scriptsize{$#1$}};
\end{tikzpicture}
}
...
\textbf{Machamp} \hfill  \fightingfull \\
\multicolumn{1}{c}{\multirow{6}{*}{\includegraphics[width=2cm]{../../images/pokemon/machamp.png}}} & Base ATK &  \attack{234} &   DPS &   \dps{16.21} \\ 
&Base DEF & \defense{159}&  Health &\survival{26.81}   \\
&Base HP & \stamina{207} &     TDO &\tdo{434.6} \\ %\cline{2-3}
&\multirow{2}{*}{Movesets}   & \fightingsimp~Counter (12)  & Remarks & \multirow{3}{4cm}{Machamp is the best \fightingfull~Pok\'emon until gen 5 \\} \\
&  					        &\fightingsimp~Dynamic Punch (90)  \\
&Weak to & \psychicfull~\fairyfull~\flyingfull&   \\
\end{tabular}
\medskip
...
\begin{itemize}
\item \textbf{Other viable alternatives}: \textbf{Breloom} (more DPS than Machamp, but very low EH), \textbf{Mewtwo} (with Focus Blast, DPS: 13.64, good EH, and not weak to \psychicfull~moves)
